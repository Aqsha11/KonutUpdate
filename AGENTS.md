...
- Headline auto-expire after 7 days, breaking after 3 days via `posts:expire-flags`, scheduled daily in `routes/console.php`. `is_featured` (Konten Pilihan) is **manual-only** — never auto-expired by the command. Post scopes (`headline()`, `breaking()`, `excludeHeadline()`) filter on `*_expires_at`, not the bare boolean — when toggling a flag manually, also set the expiry timestamp or the post drops out immediately. Scheduler needs cron; `render.yaml` has no scheduler/worker service.
