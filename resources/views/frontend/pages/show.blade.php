...
@push('styles')
<style>
    .page-content h1 { font-size: 1.5rem; font-weight: 800; margin-top: 1.5rem; margin-bottom: 0.75rem; color: var(--color-on-surface); line-height: 1.3; }
    .page-content h2 { font-size: 1.25rem; font-weight: 700; margin-top: 1.5rem; margin-bottom: 0.75rem; color: var(--color-on-surface); line-height: 1.4; padding-bottom: 0.5rem; border-bottom: 2px solid var(--color-primary); display: inline-block; }
    .page-content h3 { font-size: 1.125rem; font-weight: 600; margin-top: 1.25rem; margin-bottom: 0.5rem; color: var(--color-on-surface); line-height: 1.5; }
    .page-content h4 { font-size: 1rem; font-weight: 600; margin-top: 1.25rem; margin-bottom: 0.5rem; color: var(--color-on-surface); }
    .page-content p { font-size: 0.9375rem; line-height: 1.8; margin-bottom: 1rem; color: var(--color-on-surface-variant); }
    ...
    .page-content li { margin-bottom: 0.5rem; font-size: 0.9375rem; line-height: 1.7; color: var(--color-on-surface-variant); }
    .page-content img { border-radius: 0.75rem; margin: 1.5rem 0; max-width: 100%; height: auto; }
    .page-content blockquote { border-left: 4px solid var(--color-primary); padding: 1rem 1.25rem; font-style: italic; margin: 1.5rem 0; background: var(--color-surface-container); border-radius: 0 0.75rem 0.75rem 0; color: var(--color-on-surface-variant); }
    .page-content hr { border: none; border-top: 1px solid var(--color-outline); margin: 2rem 0; }
    .page-content strong { color: var(--color-on-surface); font-weight: 600; }
    .page-content table { width: 100%; border-collapse: collapse; margin: 1.5rem 0; font-size: 0.875rem; }
    .page-content th, .page-content td { padding: 0.75rem 1rem; border: 1px solid var(--color-outline); text-align: left; }
    .page-content th { background: var(--color-surface-container); font-weight: 600; color: var(--color-on-surface); }
    .page-content td { color: var(--color-on-surface-variant); }
    @media (max-width: 768px) {
        .page-content h1 { font-size: 1.25rem; }
        .page-content h2 { font-size: 1.1rem; }
        .page-content p { font-size: 0.85rem; line-height: 1.7; }
        .page-content li { font-size: 0.85rem; }
    }
</style>
@endpush
    ...
    <div class="relative bg-gradient-to-br from-primary/90 via-primary to-primary-dark rounded-xl lg:rounded-2xl overflow-hidden mb-8">
        ...
        <div class="relative px-6 py-10 md:px-12 md:py-16 text-center">
            <div class="w-14 h-14 md:w-16 md:h-16 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <i data-lucide="{{ $heroIcon }}" class="w-7 h-7 md:w-8 md:h-8 text-white"></i>
            </div>
            <h1 class="text-2xl md:text-4xl font-extrabold text-white mb-3">{{ $page->title }}</h1>
            <p class="text-white/80 text-base md:text-lg max-w-xl mx-auto">{{ $heroDesc }}</p>
        </div>
    </div>
