    ...
    <div class="flex flex-col lg:flex-row gap-4 lg:gap-6">
        <div class="lg:w-[68%]">
            @if($posts->count() > 0)
                ...
                {{-- Hero Section — only on page 1 (top 3), manual scroll (no autoplay) --}}
                @if($headlineItems->count() > 0)
                <section class="trending-hero mb-3 lg:mb-4">
                    <div class="hero-carousel" x-data="{ active: 0, total: {{ $headlineItems->count() }} }">
                        <div class="hero-carousel-track" :style="'transform: translateX(-' + (active * 100) + '%)'">
                            @foreach($headlineItems as $slideIndex => $big)
                            @php
                                $smalls = $headlineItems->skip($slideIndex + 1)->take(2);
                            @endphp
                            <div class="hero-carousel-slide">
                                <div class="hero-slide-grid">
                                    <div class="hero-slide-big">
                                        <span class="hero-slide-rank hot">{{ $slideIndex + 1 }}</span>
                                        <a href="{{ route('posts.show', $big->slug) }}">
                                            <img src="{{ $big->thumbnail ? Storage::url($big->thumbnail) : ($big->video_poster ?? 'https://placehold.co/800x500/1a1a2e/ffffff?text=VIDEO') }}" alt="{{ $big->title }}" class="hero-slide-big-img" loading="{{ $loop->first ? 'eager' : 'lazy' }}">
                                        </a>
                                    ...
                                    <div class="hero-slide-smalls">
                                        ...
                                        <div class="hero-slide-small">
                                            <span class="hero-slide-rank">{{ $slideIndex + 1 + $loop->iteration }}</span>
