...
<div class="page-header">
    <div>
        <h1>Edit Berita</h1>
        <p class="page-subtitle">Perbarui berita yang sudah ada</p>
    </div>
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="form-group">
                    ...
                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" placeholder="Judul Berita" value="{{ old('title', $post->title) }}" required>
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <textarea name="excerpt" id="excerpt" rows="3" class="form-control @error('excerpt') is-invalid @enderror" placeholder="Ringkasan berita (opsional)">{{ old('excerpt', $post->excerpt) }}</textarea>
                    @error('excerpt')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <textarea name="body" id="editor" rows="12" class="form-control @error('body') is-invalid @enderror" placeholder="Tulis konten berita di sini..." style="display:none;">{{ old('body', $post->body) }}</textarea>
            ...
            <div class="col-md-4">
                ...
                <div class="form-group" id="videoFields" style="display: {{ old('type', $post->type) === 'video' ? 'block' : 'none' }};">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="tab-upload">
                            @if($post->video_path && !str_starts_with($post->video_path, 'http'))
                                <div class="mb-2">
                                    <video src="{{ Storage::url($post->video_path) }}" controls class="w-100 rounded" style="max-height:200px;"></video>
                                    <p class="text-muted mt-1 mb-0" style="font-size:0.75rem;">Video saat ini</p>
                                </div>
                            @endif
                        ...
                        <div class="tab-pane fade" id="tab-url">
                            <input type="url" name="video_url" id="video_url" class="form-control @error('video_url') is-invalid @enderror" value="{{ old('video_url', str_starts_with($post->video_path ?? '', 'http') ? $post->video_path : '') }}" placeholder="https://youtube.com/watch?v=...">
                            ...
                            @if($post->video_path && str_starts_with($post->video_path, 'http'))
                                <div class="mt-2">
                                    <p class="text-muted mb-1" style="font-size:0.75rem;">URL video saat ini:</p>
                                    <a href="{{ $post->video_path }}" target="_blank" class="text-truncate d-block" style="font-size:0.75rem;">{{ $post->video_path }}</a>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="border rounded p-3 @error('category_ids') border-danger @enderror" style="max-height:200px;overflow-y:auto;">
                        ...
                        <div class="form-check mb-1">
                            <input type="checkbox" name="category_ids[]" value="{{ $category->id }}" id="cat_{{ $category->id }}" class="form-check-input category-checkbox" {{ in_array($category->id, old('category_ids', $post->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="tags" id="tags" class="form-control" value="{{ old('tags', $post->tags->pluck('name')->implode(', ')) }}" placeholder="pisahkan dengan koma">
                ...
                <div class="form-group">
                    ...
                    <div class="d-flex gap-3 flex-wrap">
                        <div class="form-check">
                            <input type="radio" name="status" id="statusDraft" value="draft" class="form-check-input" {{ old('status', $post->status) === 'draft' ? 'checked' : '' }}>
                    ...
                    <div class="d-flex gap-4 flex-wrap">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <label class="toggle-switch mb-0">
                                ...
                                <input type="checkbox" name="is_headline" id="isHeadline" value="1" {{ old('is_headline', $post->is_headline) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" name="published_at" id="published_at" class="form-control" placeholder="Tanggal Publikasi" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '') }}">
                </div>
